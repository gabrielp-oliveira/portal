...
import { PaperTableComponent } from '../paper/paper-table.component';
...
export class ChapterComponent implements OnInit, OnDestroy {
  displayedColumns: string[] = ['order', 'name', 'created_at',  'paperName', "timelineName", 'storylineName','options'];
  ...
  openChapter(paperId: string){
    const url = `/world/${this.wd.worldId}/chapter/${paperId}`;
  ...
  compareUpdates(): boolean {
    ...
    const valPaperName = this.paperNameSearchValue == ""
  ...
  ngOnInit() {
    ...
    .subscribe((data) => {
      ...
      const chp:ExtendedChapter[] = chapters.map((c) => {
        ...
        cht.paperName = pp?.name || ''
        cht.paperOrder = pp?.order || 1
...
// Verifica se o subwayElement contém o targetElement como filho
  ...
  sortByOrder(a: ExtendedChapter, b: ExtendedChapter): number {
    ...
    if (a.paperOrder !== undefined && b.paperOrder !== undefined) {
      return this.sortDirection ? a.paperOrder - b.paperOrder : b.paperOrder - a.paperOrder;
    }
  ...
  sortByPaperName(a: ExtendedChapter, b: ExtendedChapter): number {
    if (this.sortDirection) {
  ...
  searchChapter(key: string) {
    switch (key) {
      ...
      case 'name':
        ...
        return
      case 'paperName':
        let paperName = this.paperNameSearchValue.toLowerCase(); // Normalizar para minúsculas
        if (paperName && paperName != "") {
          const data = this.dataSource.data.filter((c) =>
            c.paperName?.toLowerCase().includes(paperName)
          );     
          this.dataSource.data = data
          this.setGlobalData(data, paperName)
  ...
  returnSortChapters(criteria: string, data:ExtendedChapter[]) {
    switch (criteria) {
      case 'order':
        ...
        case 'paperName':
        this.isDrag = false
        return data.sort((a: ExtendedChapter, b: ExtendedChapter) => this.sortByPaperName(a, b));
        case 'timelineName':
        this.isDrag = false
